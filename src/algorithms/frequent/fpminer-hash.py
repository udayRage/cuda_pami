"""
CUDA Frequent Pattern Miner
=======================================

Highlights in this refactor
---------------------------
- Replaced hash table with a hash set for efficient membership testing.
- Removed utility/probability calculations; support is now based on frequency.
- Implemented a parallel sum reduction in the mining kernel for efficient support counting.
- Retained the single, switchable allocator for all GPU memory.
- Kept optional pinned host pool and deterministic GDS vs POSIX I/O toggling.
- Maintained peak memory telemetry features.
- Ensured safer kernel compilation and lazy cudf import.

Note: Input is expected to be a list of transactions.
"""
...
HashSetEntry = np.dtype([("line", np.uint32)], align=True)
...
struct HashSetEntry { uint32_t line; };
...
__device__ __forceinline__ void hs_insert(hash_set &hs, uint32_t key) {
    uint32_t h = mul_hash(key) & hs.mask;
    while (true) {
        uint32_t old = atomicCAS(&hs.table[h].line, SENTINEL, key);
        if (old == SENTINEL || old == key) { break; }
        h = (h + 1u) & hs.mask;
    }
}
extern "C" __global__ void build_sets(uint64_t total, const uint32_t *items, const uint32_t *lines, hash_set* hss) {
    uint32_t tid = threadIdx.x + blockIdx.x * blockDim.x; if (tid >= total) return;
    hs_insert(hss[items[tid]], lines[tid]);
}
__device__ bool hs_contains(const hash_set &hs, uint32_t key) {
    uint32_t h = mul_hash(key) & hs.mask;
    while (true) {
        uint32_t cur = hs.table[h].line; if (cur == key) return true;
        if (cur == SENTINEL) return false; h = (h + 1u) & hs.mask;
    }
}

extern "C" __global__ void mine_kernel(
    const hash_set* __restrict__ hss,
    ...
    const hash_set& hsEnd = hss[cands[bid*candSize + candSize - 1]];
    uint32_t reqHits = candSize - 1u;

    for (uint32_t i = tid; i < hsEnd.arr_len; i += blockDim.x) {
        uint32_t line = hsEnd.arr[i].line;
        uint32_t hits = 0u;
        #pragma unroll 1
        for (uint32_t j = 0; j < reqHits; ++j) {
            if (hs_contains(hss[cands[bid*candSize + j]], line)) {
                hits++;
            } else {
                break;
            }
    ...
    if (tid == 0) {
        atomicAdd(&supports[bid], s_support[0]);
    }
}
...
# -----------------------
class cuFPMiner(BaseAlgorithm):
    """
    cuDF/CUDA-based frequent pattern miner.
    ...
    def _calculate_support_and_filter_items(self, gdf):
        ...
        support_df = support_df[support_df["freq"] >= self._minSup].sort_values("freq", ascending=False).reset_index()
        if len(support_df) == 0:
            return None, None
    ...
    def _allocate_gpu_hash_sets(self, support_df):
        ...
        self.buckets = cp.zeros(slots_total, dtype=HashSetEntry)
        base_ptr = self.buckets.data.ptr
        offset = cp.cumsum(cp.concatenate([cp.array([0], dtype=cp.uint64),
                                        sizes_cp.astype(cp.uint64)])) * HashSetEntry.itemsize

        dense_total = int(freq_cp.sum())
        self.dense = cp.empty(dense_total, dtype=HashSetEntry)
        dense_base = self.dense.data.ptr
        dense_offs = cp.cumsum(cp.concatenate([cp.array([0], dtype=cp.uint64),
                                            freq_cp.astype(cp.uint64)])) * HashSetEntry.itemsize

        hs_host = np.zeros(self.max_id + 1, dtype=HashSet)
        sizes_host   = sizes_cp.get()
        offset_host  = offset.get()
        freq_host    = freq_cp.get()
        dense_offs_h = dense_offs.get()

        for idx, id_ in enumerate(support_df["new_id"].to_numpy()):
            hs_host[id_]["table_ptr"]  = base_ptr + offset_host[idx]
            hs_host[id_]["table_size"] = sizes_host[idx]
            hs_host[id_]["mask"]       = sizes_host[idx] - 1
            hs_host[id_]["id"]         = id_
            hs_host[id_]["array_ptr"]  = dense_base + dense_offs_h[idx]
            hs_host[id_]["array_size"] = freq_host[idx]

        self._max_threads = min(int(sizes_cp.min()), THREADS) if len(sizes_cp) > 0 else THREADS
        self._snap_mem()
        return hs_host

    def _build_gpu_hash_sets(self, hs_host, gdf):
        
        self.item_col = gdf["item"].astype("uint32").values
        self.line_col = gdf["txn_id"].astype("uint32").values

        self._hs_dev = self._to_device_struct(hs_host)

        total_pairs = len(self.item_col)
        grid_size = (total_pairs + self._max_threads - 1) // self._max_threads

        self._kern_build((grid_size,), (self._max_threads,),
                        (total_pairs, self.item_col, self.line_col, self._hs_dev))

        self._arr_counters = cp.zeros(self.max_id + 1, dtype=cp.uint32)
        self._kern_write_dense((grid_size,), (self._max_threads,),
                            (total_pairs, self.item_col, self.line_col, self._hs_dev, self._arr_counters))
    ...
    def _mine(self):
        ...
        hs_host = self._allocate_gpu_hash_sets(support_df)
        self._build_gpu_hash_sets(hs_host, final_gdf)
        ...
        while len(candidates) > 0:
            ...
            self._kern_mine((len(next_candidates),), (self._max_threads,), (self._hs_dev, next_candidates, len(next_candidates), k, supports), shared_mem=self._max_threads * 4)
            cp.cuda.runtime.deviceSynchronize()
            self._snap_mem()
...
def _cli():
    p = argparse.ArgumentParser(description="cuFPMiner for frequent pattern mining")
