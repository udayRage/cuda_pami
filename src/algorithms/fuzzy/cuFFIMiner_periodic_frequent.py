...
try:
    import pynvml as _nvml  # optional
...
extern "C" __global__ void write_dense_arrays(
    ...
    const uint64_t* __restrict__ probs, hash_table* __restrict__ hts, uint32_t* __restrict__ counters){
    uint32_t tid = threadIdx.x + blockIdx.x * blockDim.x; if (tid >= total) return;
    uint32_t it = items[tid]; uint32_t pos = atomicAdd(&counters[it], 1u);
...
extern "C" __global__ void build_tables(uint64_t total, const uint32_t *items, const uint32_t *lines, const uint64_t *probs, hash_table* hts){
    uint32_t tid = threadIdx.x + blockIdx.x * blockDim.x; if (tid >= total) return; ht_insert(hts[items[tid]], lines[tid], probs[tid]); }

__device__ uint64_t ht_get(const hash_table &ht, uint32_t key){
    uint32_t h = mul_hash(key) & ht.mask; while (true){ uint32_t cur = ht.table[h].line; if (cur == key) return ht.table[h].probability; if (cur == SENTINEL) return 0u; h = (h + 1u) & ht.mask; } }

extern "C" __global__ void mine_kernel(const hash_table* __restrict__ hts, const uint32_t* __restrict__ cands, uint32_t nCands, uint32_t candSize, uint64_t* __restrict__ supports, uint32_t* __restrict__ freqs)
    uint32_t bid = blockIdx.x; if (bid >= nCands) return; const hash_table& htEnd = hts[cands[bid*candSize + (candSize - 1)]];
    uint32_t reqHits = candSize - 1u; uint64_t local_support = 0; uint32_t local_freq = 0;
    for (uint32_t i = threadIdx.x; i < htEnd.arr_len; i += blockDim.x){ uint32_t line = htEnd.arr[i].line; uint64_t min_p = htEnd.arr[i].probability; uint32_t hits = 0u;
        #pragma unroll 1
        ...
        if (hits == reqHits){ local_support += min_p; local_freq += 1; } }
    if (local_support != 0){ atomicAdd(&supports[bid], local_support); atomicAdd(&freqs[bid], local_freq); } }
...
# -----------------------
# Allocator, GDS, kernels
# -----------------------
def _set_kvikio_mode(mode: str) -> None:
    """Set KVIKIO_COMPAT_MODE (auto/on/off) for cuFile (GDS) vs POSIX I/O."""
    m = mode.lower()
...
# -----------------------
class cuFFIMiner_periodic_frequent(BaseAlgorithm):
    """GPU/cuDF fuzzy frequent itemset miner for pre-quantized integer probabilities."""

    def __init__(
        self,
        iFile: Union[str, Path],
        min_support: int,
        min_periodic_frequency: int,
        min_period: int,
        sep: str = "\t",
        ...
        """Configure inputs, allocator pools, GDS mode, debug flags, and compile kernels."""
        super().__init__(iFile=iFile, debug=debug)
        self.debug = bool(debug)
        self._sep = sep
        self._minSup_scaled = int(min_support)
        self._minPeriodicFreq = int(min_periodic_frequency)
        self._minPeriod = int(min_period)
    ...
    @staticmethod
    def _mb(x: int) -> str:
        """Format bytes as MB with 3 decimals."""
        return f"{x/1024**2:,.3f}"

    def _static_bytes_post_build(self) -> Dict[str, int]:
        """Compute sizes of long-lived structures after build()."""
        return {
    ...
    def _print_static_breakdown(self, static_map: Dict[str, int]) -> None:
        """Print static breakdown and store it for getters (debug-only print)."""
        self._theory_static_map = dict(static_map)  # keep for getters
        if not self.debug:
            return
    ...
    def _record_manual_report(
        ...
        *,
        ...
        """Record a manual-memory snapshot and optionally print it."""
        ...
        if self.debug:
            ...
            print(
                f"[theory/{tag}] static={mbv('static')} MB | start={mbv('start')} MB | gen_peak={mbv('gen_peak')} MB | "
                f"mine_peak={mbv('mine_peak')} MB | steady_post={mbv('steady_post')} MB"
            )
            print(
                "           components:"
    ...
    # -------- Optional telemetry peaks (summary only) --------
    def _init_nvml(self) -> None:
        """Initialize NVML handle for per-process memory telemetry if available."""
        self._nvml = None
        ...
        try:
            if _nvml is None:
                return
            _nvml.nvmlInit()
            dev_index = cp.cuda.runtime.getDevice()
            self._nvml_handle = _nvml.nvmlDeviceGetHandleByIndex(dev_index)
            self._nvml = _nvml
        except Exception:
            self._nvml = None

    def _snap_mem(self) -> None:
        """Update peak counters using driver/pool info for end-of-run summary."""
        try:
    ...
    def _load_text(self, path: Union[str, Path]):
        """Load colon-separated transactions into cuDF with per-line txn_id and string lists."""
        import cudf
        gdf = cudf.read_csv(str(path), sep=":", header=None, names=["items", "values"], dtype=["str", "str"])
    ...
    def _explode_transactions(self, gdf):
        """Explode item/probability lists into long format with aligned positions."""
        import cudf
        ...
        out = items_ex.merge(values_ex, on=["txn_id", "pos"]).drop(columns="pos")
        try:
            out["prob"] = out["values"].astype("uint32")
        except Exception as e:
            raise ValueError("Values in text file must be convertible to uint32.") from e
        return out.drop(columns="values").rename(columns={"items": "item"})

    def _load_parquet(self, path: Union[str, Path]):
        """Load parquet with required columns {item, prob, txn_id} and correct dtypes."""
        import cudf
        gdf = cudf.read_parquet(str(path))
        req = ["item", "prob", "txn_id"]
    ...
    def _calculate_support_and_filter_items(self, gdf):
        """Compute supports, filter by threshold, assign dense IDs, and remap data."""
        ...
        gdf = gdf.merge(support_df[["item", "new_id"]], on="item").drop(columns=["item"]).rename(columns={"new_id": "item"})
    ...
    def _allocate_gpu_hash_tables(self, support_df):
        """Allocate per-item open-addressing tables and dense arrays as contiguous slabs."""
        ...
        sizes_h = sizes_cp.get(); offset_h = offset.get(); freq_h = freq_cp.get(); dense_off = dense_offs.get()
    ...
    def _build_gpu_hash_tables(self, ht_host, gdf) -> None:
        """Transfer columns to GPU, build hash tables, and backfill dense arrays."""
        ...
        self._kern_build((grid,), (self._max_threads,), (total_pairs, self.item_col, self.line_col, self.prob_col, self._ht_dev))
        self._arr_counters = cp.zeros(self.max_id + 1, dtype=cp.uint32)
        self._kern_write_dense((grid,), (self._max_threads,), (total_pairs, self.item_col, self.line_col, self.prob_col, self._ht_dev, self._arr_counters))
        cp.cuda.runtime.deviceSynchronize()
        self._snap_mem()
    ...
    def _mine(self) -> None:
        """Execute: load → explode → filter → build → iterate → collect, with manual memory reports."""
        ...
        while len(candidates) > 0:
            ...
            k += 1
            supports = cp.zeros(len(next_candidates), dtype=cp.uint64)  # temp
            freqs = cp.zeros(len(next_candidates), dtype=cp.uint32)    # temp
            self._kern_mine((len(next_candidates),), (self._max_threads,), (self._ht_dev, next_candidates, len(next_candidates), k, supports, freqs))
            cp.cuda.runtime.deviceSynchronize()

            mask = (supports >= self._minSup_scaled) & (freqs >= self._minFreq)                  # temp
            frequent = next_candidates[mask]                        # becomes next candidates

            self._record_manual_report(
                f"k={k}/mine", static_map=static_map, candidates=candidates,
                next_candidates=next_candidates, supports=supports, mask=mask, frequent=frequent
            )
    ...
    def _process_results(self, gpu_results) -> None:
        ...
        patterns = {
            (item,): float(prob) / float(self._scaling_factor)
        ...
        for ids_cpu, vals in gpu_results:
            ...
            for name_tuple, sup in zip(names, vals):
                patterns[tuple(name_tuple)] = float(sup) / float(self._scaling_factor)
...
def _cli() -> None:
    ...
    p = argparse.ArgumentParser(description="cuFFIMiner (manual-memory) for pre-scaled integer data")
    ...
    p.add_argument("min_periodic_frequency", type=int, help="Minimum periodic frequency threshold")
    p.add_argument("min_period", type=int, help="Minimum period threshold")
    p.add_argument("scaling_factor", type=int, help="Scale factor to divide supports in output")
    ...
    p.add_argument("--allocator", type=str, default="rmm_device",
    ...
    miner = cuFFIMiner_periodic_frequent(
        iFile=args.iFile,
        min_support=args.min_support,
        min_periodic_frequency=args.min_periodic_frequency,
