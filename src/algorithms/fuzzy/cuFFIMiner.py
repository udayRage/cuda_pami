...
extern "C" __global__ void mine_kernel(const hash_table* __restrict__ hts, const uint32_t* __restrict__ cands, uint32_t nCands, uint32_t candSize, uint64_t* __restrict__ supports){
...
class cuFFIMiner(BaseAlgorithm):
    ...
    def _mine(self) -> None:
        ...
        while len(candidates) > 0:
            ...
            self._kern_mine((len(next_candidates),), (self._max_threads,), (self._ht_dev, next_candidates, len(next_candidates), k, supports))
            cp.cuda.runtime.deviceSynchronize()

            mask = supports >= self._minSup_scaled                  # temp
