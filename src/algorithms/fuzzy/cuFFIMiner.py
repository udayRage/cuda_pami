"""
cuFFIMiner - CUDA Fuzzy Frequent Itemset Miner (refactored)
===========================================================

Highlights in this refactor
---------------------------
- Single, switchable allocator for *all* GPU memory:
  - RMM device pool (default), RMM managed (UVM) pool, CuPy device pool, CuPy managed (UVM) pool
- Optional pinned host pool
- Deterministic GDS vs POSIX I/O toggling via KVIKIO_COMPAT_MODE (auto/on/off)
- Peak memory telemetry:
  - CUDA driver peak (used bytes)
  - Pool peaks (used/total)
  - RMM statistics peak_bytes (if available)
- Safer kernel compile (no duplicate RawModule)
- Lazy cudf import so env/allocators are set first

Note: Input still assumes **pre-scaled uint32** probabilities for this miner variant.
"""
...
# Import everything from the base module (stop on purpose if missing)
...
class cuFFIMiner(BaseAlgorithm):
    """
    cuDF/CUDA-based fuzzy FIM assuming pre-scaled uint32 probabilities.
    ...
    def _load_text(self, path: Union[str, Path]):
        ...
        gdf = cudf.read_csv(str(path), sep=":", header=None,
                            names=["items", "values"], dtype=["str", "str"])
    ...
    def _build_gpu_hash_tables(self, ht_host, gdf):
        ...
        cp.cuda.runtime.deviceSynchronize()
        self._snap_mem()
        # Optional managed prefetch of big slabs to the current device
...
def _cli():
    p = argparse.ArgumentParser(description="cuFFIMiner (refactored) for pre-scaled integer data")
