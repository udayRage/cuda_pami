...
# -----------------------
class cuFFIMiner_frequent(BaseAlgorithm):
    ...
    def __init__(
        ...
        self._minFreq = int(min_frequency)
    ...
    def _calculate_support_and_filter_items(self, gdf):
        ...
        support_df = support_df[(support_df["prob"] >= self._minSup_scaled) & (support_df["freq"] >= self._minFreq)].sort_values("freq", ascending=False).reset_index()
        if len(support_df) == 0:
            return None, None
...
def _cli() -> None:
    ...
    p.add_argument("min_freq", type=int, help="Minimum frequency threshold")
